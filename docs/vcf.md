...
We have repurposed the sample column to report haplotypes (gene copies) found in a region (gene family). Each column represents a haplotype. The haplotype names are consistent with those reported in the `json` [file](json.md). 
...
* `GT` (Genotype): the called genotype of that haplotype, with 0=REF and 1=ALT.
* `DP` (total depth): a single number corresponding to the total number of reads that spanned the locus.
