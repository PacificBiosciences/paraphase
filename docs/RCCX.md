...
Fields shared across all genes are defined in the general json [file](json.png). The RCCX module includes several unique fields, listed below:

- `ending_hap`: Indicates the last RCCX copy on each allele. Only these final copies typically contain the gene TNXB; all earlier copies on the same haplotype contain TNXA (the pseudogene).
...
- `annotated_alleles`: Provides per-allele annotations of CYP21A2 based on the `hap_variants` field. Possible values include:
