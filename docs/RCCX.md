...
- `alleles_final`: haplotypes phased into two alleles.
- `phasing_success`: Indicates whether the different haplotypes could be phased into alleles. Note that this does not refer to haplotype phasing itself; haplotypes (i.e.individual RCCX copies) can still be resolved even if they cannot be connected into alleles.
...
Notes regarding alleles: When possible, the order of haplotypes in each allele reported in `alleles_final` reflects the order of RCCX modules on an allele, i.e. starting with a starting copy (one of those in `starting_hap`) and ending with an ending copy (one of those in `ending_hap`). `alleles_final` and `raw_alleles` have identical values for the `RCCX` region and they are both updated to empty values when `phasing_success` is false. 
